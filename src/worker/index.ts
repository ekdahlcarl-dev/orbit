...
export async function runWorkerIteration(db = getDb(), github = new GitHubClient()): Promise<void> {
  ...
  try {
    ...
    const result = await client.query(`SELECT id,job_type,payload,attempts FROM job_queue WHERE status='queued' AND available_at<=now() ORDER BY id LIMIT 1 FOR UPDATE SKIP LOCKED`);
    ...
    try {
      if (job.job_type === "github.webhook") await processWebhook(client, job.payload.deliveryId, github);
      else if (job.job_type === "build.trigger") await processBuildTrigger(client, github, job.payload);
      ...
      await client.query(`UPDATE job_queue SET status=$2,available_at=now()+interval '30 seconds',updated_at=now() WHERE id=$1`, [job.id, job.attempts + 1 >= 3 ? "failed" : "queued"]);
    ...
    await client.query("ROLLBACK").catch(() => undefined); logger.error({ err: error }, "worker iteration failed"); throw error;
  } finally { client.release(); }
}

if (import.meta.url === `file://${process.argv[1]}`) {
  const interval = Number(process.env.WORKER_POLL_MS ?? 2000); logger.info({ interval }, "ORBIT worker started"); let running = false;
  setInterval(async () => { if (running) return; running = true; try { await runWorkerIteration(); } catch { } finally { running = false; } }, interval);
}
